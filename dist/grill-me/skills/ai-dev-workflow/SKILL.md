...
Honor an explicit user selection of `$grill-requirements`, `$grill-with-docs`, `$write-spec`, `$plan-tickets`, `$implement-tdd`, `$review-code`, or `$improve-architecture` over automatic routing (`ROUTE-USER-001`). Preserve every capability, safety, and approval gate; direct selection cannot bypass a missing prerequisite.
...
Use `$grill-requirements` for a fresh, self-contained requirement unless the user selects another mode. Select `$grill-with-docs` automatically when:
...
| --- | --- |
| Important decisions remain ambiguous | Choose the requirement mode, then invoke `$grill-requirements` or `$grill-with-docs` |
...
When an Approved or accepted artifact introduces, changes, supersedes, or resolves durable facts, propose a Knowledge Base Change Summary through `$grill-with-docs` rules. Show additions, modifications, and removals and bind approval only to the displayed changes. Do not invent a knowledge update when the artifact adds no durable fact.

## Migrate v2 on first use

When a project has approved v2 artifacts but no v3 Project Knowledge Base:

For this migration, propose an initial Project Knowledge Base only from approved evidence.

1. Inspect the available approved v2 artifacts without editing them.
...
Migration must not rewrite, relabel, or overwrite approved v2 artifacts (`MIGRATE-V2-001`). If migration is rejected or fails, leave v2 evidence unchanged and continue without an active v3 Knowledge Base.
