...
Emit an Architecture Improvement Report with `artifact_type`, stable `artifact_id`, shared `workflow_id`, `core_version` `3.0.0`, `status`, `inputs`, `assumptions`, `deferred`, `handoff`, and `approval` evidence when accepted.
