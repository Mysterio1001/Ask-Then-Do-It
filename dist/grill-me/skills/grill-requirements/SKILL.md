...
At consolidation, emit a Requirement Decision Record with `status` set to `Draft`. Include or unambiguously convey `artifact_type`, stable `artifact_id`, shared `workflow_id`, `core_version` `3.0.0`, upstream `inputs`, `assumptions`, `deferred` decisions, the next-stage `handoff`, and empty or pending `approval` evidence.
