...
Include or unambiguously convey `artifact_type` as Ticket Plan, a stable `artifact_id`, the Specification's `workflow_id`, `core_version` `1.0.1`, `status`, upstream `inputs`, `assumptions`, `deferred` decisions, the next-stage `handoff`, and `approval` evidence. Preserve these values when revising an existing artifact.
