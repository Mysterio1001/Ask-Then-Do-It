...
Maintain Draft Working Notes with `status` fixed to `Draft`. Include `artifact_type`, stable `artifact_id`, shared `workflow_id`, `core_version` `1.4.0`, `status`, `inputs`, `assumptions`, `deferred`, `handoff`, and pending `approval`.
