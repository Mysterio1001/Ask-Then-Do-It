...
Emit Direct Implementation Evidence only for work actually performed with the `tools` profile. Include or unambiguously convey `artifact_type`, stable `artifact_id`, shared `workflow_id`, `core_version` `1.3.1`, evidence `status`, upstream `inputs`, `assumptions`, `deferred` work, and reviewer `handoff`.
