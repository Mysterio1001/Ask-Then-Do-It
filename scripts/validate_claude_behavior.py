...
def contracts() -> tuple[dict, dict, dict]:
    ...
    require((manifest["adapter_id"], manifest["adapter_version"], manifest["target"], manifest["core_version"], manifest["status"])
            == ("claude-code", "1.4.0", "claude-code-plugin", "1.4.0", "staged-unverified"), "staged conformance identity/status mismatch")
...
def prepare(output: Path, plugin_root: Path = PLUGIN) -> Path:
    ...
    ledger = {
        "schema_version": 1, "target_version": "1.4.0", "evidence_scope": "model-profile-response",
...
def _validate(path: Path, plugin_root: Path, kind: str) -> list[str]:
    try:
        ...
        require(type(ledger["schema_version"]) is int and ledger["schema_version"] == 1 and ledger["target_version"] == "1.4.0", "ledger version mismatch")
