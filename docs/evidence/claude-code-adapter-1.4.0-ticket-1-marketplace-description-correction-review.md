...
Inputs: Approved [Requirement Decision Record](../specs/claude-code-adapter.md)的 Marketplace／provider-boundary 與 strict-validation requirements；Approved corrected [Specification](../specs/claude-code-adapter.md)第 2 節及 acceptance criteria 2、3；Approved [Ticket Plan](../specs/claude-code-adapter.md) Ticket 1；[TDD correction evidence](../project/status.md)；final `.claude-plugin/marketplace.json`、`scripts/validate_claude_plugin.py`、`tests/claude/test_public_plugin_contract.py`、canonical Claude `plugin.json`，以及 Codex catalog／validator／tests 的 surrounding provider contracts；使用者提供的 raw focused、affected、full、static 與 native verification results。
