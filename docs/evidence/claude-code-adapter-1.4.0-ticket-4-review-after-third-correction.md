...
Inputs: Approved [Specification](../specs/claude-code-adapter.md), Approved [Ticket Plan, Ticket 4](../specs/claude-code-adapter.md), `core/rules/rules.yaml`, all ten final General modules, `tests/claude/test_general_profile.py`, all three `tests/claude/fixtures/general-profile/` files, both public Skill bootstraps, the relevant shared validator and public contract tests, the raw third-correction verification record in [Ticket 4 evidence](../project/status.md), and the subsequent direct-architecture lens correction and its supplied raw Red/Green results.
