...
整理日期：2026-09-10；本次版本目標更新：2026-09-21。此頁以當前來源、保留的正式文件與發布收據彙整進度；歷史結果不當成本次重跑結果。使用者核准將當前來源更新至 stable 1.4.3，不覆寫既有 `v1.4.2` tag；1.4.3 的 commit、push、tag 與 GitHub Release 尚未完成。Claude live qualification 是選配且目前 `not run`／`unverified`。

## 已交付與證據邊界

| 項目 | 狀態 | 可支持的結論 |
| --- | --- | --- |
| Core、Codex、Generic、Claude 來源版本 | 1.4.3 已對齊 | 版本、catalog、consumer identity 與文件需在 staged diff 再核對；一次性 Ticket evidence 已移出 repository，不作現行 gate |
...
| 目標 stable 1.4.3 | 尚未發布 | 1.4.2 required ledger／evidence 僅保留為歷史證據；1.4.3 仍須完成精準 commit、exact-commit clean rebuild 兩次、required evidence gate、non-force push、新 tag／Release 與公開下載 checksum 驗證 |
...
| 5 | 建立 annotated `v1.4.3`、stable GitHub Release，公開下載三個 ZIP／checksums 並重驗 | 任何同版衝突或 required gate 失敗即停止，不改寫既有 `v1.4.2` tag／assets |
...
- `.claude-offline/verification/`：歷史狀態曾記錄 prepared behavior ledger／prompts，但目前路徑實測不存在；不得把該歷史描述當成本次 1.4.3 的可用 evidence。若日後另行授權 T4 optional qualification，才建立全新、不可覆寫的實測根目錄。
