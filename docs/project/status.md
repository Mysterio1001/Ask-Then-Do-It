...
整理日期：2026-09-10；版本目標更新：2026-09-11。此頁以當前來源、已保留的 Review、發布收據及原始離線證據彙整進度；歷史結果不當成本次重跑結果。文件合併與刪除已由使用者明確核准，整理本身不代表新的功能或 Claude 實測完成。使用者已核准專案、Core 與三平台套件統一升至 1.4.1，並於 2026-09-11 另行回覆「可以提交以及發布了」，授權此次提交與對外發布；授權不等於外部操作已完成，也不取代尚缺的實測證據。

## 已交付與證據邊界

| 項目 | 狀態 | 可支持的結論 |
| --- | --- | --- |
| Core、Codex、Generic、Claude 來源版本 | 1.4.1 | 統一來源與套件版本；新版驗證須對應新 candidate，不沿用舊版結果 |
| 1.4.1 本機候選包與回歸驗證 | 2026-09-11 完成 | 三平台 ZIP 位於 `ticket-release-141-build-candidate/`；396 tests、395 passed、1 Windows symlink skip。套件 inventory／checksum／來源一致性、兩次建置 81 個輸出檔案一致性及獨立 Review 通過；未實測 Claude Desktop 匯入／觸發，不等於正式 release gates 完成 |
...
| 6 | 至少一個乾淨真實環境跑完整 install、兩入口、route／switch、session、update、remove、reinstall／ZIP recovery，再做 final Review／release evidence | 承接原 Ticket 10；前述正式 gates 須針對 1.4.1 candidate 通過，才宣告完整 local 1.4.1 完成 |
