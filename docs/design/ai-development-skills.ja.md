...
Ask Then Do It は、要件の合意、仕様、Tickets、各 Ticket で利用者が選ぶ実装モード、Review、アーキテクチャ改善を明確な段階に分け、これらのリスクを管理します。
...
| Codex Plugin | Codex が選択できる 9 個の Skills としてフローを提供する |
...
縦割りの Ticket には、その結果に必要なデータ、ロジック、画面や接点をまとめて含めます。すべての Tickets を示した後、フローは各 Ticket のテスト方針と、時間およびリスクへの影響を説明します。利用者は、すべての Ticket についてテストを追加するかどうかを一度に回答できます。全部に追加、全部に追加しない、または一部だけを指定でき、すべての選択が確定してから計画を承認します。
...
利用者がテストを追加しない場合、フローは内部の `direct` 経路を使用します。すべての Ticket でテストを追加しない選択も有効です。direct は振る舞いテストを作成も実行もしませんが、lint、型チェック、build は実行できます。実装の根拠と Review には `tests: skipped-by-user` と未テストの範囲を残し、TDD と同等の信頼性は主張しません。
...
分析の受け入れは実装の許可ではありません。実際の変更は、仕様、Ticket 計画、各 Ticket での利用者のモード選択、対応する実装経路を通して改めて進めます。
