...
class AskThenDoItGenericIdentityTests(unittest.TestCase):
    def test_isolated_generic_package_has_new_identity_legal_files_and_entrypoint(self) -> None:
        ...
        self.assertEqual(
            config["generic"]["directory"], "generic/ask-then-do-it-generic-1.4.2"
        )
        ...
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary:
            ...
            package = output / "generic" / "ask-then-do-it-generic-1.4.2"
            archive = output / "generic" / "ask-then-do-it-generic-1.4.2.zip"
            ...
            self.assertRegex(guide, r"沒有從屬(?:或背書)?關係")
            ...
            for legal_file in ("LICENSE", "THIRD_PARTY_NOTICES.md"):
                ...
                with zipfile.ZipFile(archive) as bundle:
                    self.assertEqual(bundle.read(f"ask-then-do-it-generic-1.4.2/{legal_file}"), (ROOT / legal_file).read_bytes())
