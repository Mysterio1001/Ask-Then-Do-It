...
class ReleaseOneThreeContractTests(unittest.TestCase):
    ...
    def test_active_identity_and_current_document_downloads_are_current(self) -> None:
        ...
        for document in root_download_docs:
            ...
            with self.subTest(document=document.relative_to(ROOT)):
                self.assertIn("v1.4.3", body)
                self.assertIn("ask-then-do-it-1.4.3.zip", body)
                self.assertIn("ask-then-do-it-generic-1.4.3.zip", body)

        host_contracts = (
            (
                "codex",
                [ROOT / "docs" / "guides" / f"codex.{locale}.md" for locale in ("en", "zh-TW", "ja")],
                "ask-then-do-it-1.4.3.zip",
                "ask-then-do-it-generic-1.4.3.zip",
            ),
            (
                "generic",
    ...
    def test_release_config_locks_runtime_inventory_and_proxy_gate(self) -> None:
        ...
        self.assertEqual(config["codex"]["archive"], "codex/ask-then-do-it-1.4.3.zip")
        self.assertEqual(config["generic"]["directory"], "generic/ask-then-do-it-generic-1.4.3")
        self.assertEqual(config["generic"]["archive"], "generic/ask-then-do-it-generic-1.4.3.zip")
    ...
    def test_release_builder_rejects_marketplace_ref_version_drift(self) -> None:
        ...
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary:
            ...
            with mock.patch.object(builder, "MARKETPLACE_CATALOG", candidate):
                with self.assertRaisesRegex(builder.BuildError, "marketplace.*v1.4.3"):
    ...
    def test_builder_emits_exact_1_3_runtime_packages_and_reference_inventory(self) -> None:
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary:
            ...
            if result.returncode != 0:
                self.fail(f"builder did not produce the approved 1.4.3 package: {result.stderr}")

            codex = output / "codex" / "ask-then-do-it"
            generic = output / "generic" / "ask-then-do-it-generic-1.4.3"
            ...
            self.assertIn('release_version: "1.4.3"', manifest)
            self.assertIn('core_version: "1.4.3"', manifest)
            ...
            for provider, archive in (
                ("codex", "ask-then-do-it-1.4.3.zip"),
    ...
    def test_consumer_sources_packages_and_archives_exclude_pillow(self) -> None:
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary:
            ...
            for archive in (
                output / "codex" / "ask-then-do-it-1.4.3.zip",
                output / "generic" / "ask-then-do-it-generic-1.4.3.zip",
            ):
    ...
    def test_clean_builds_and_default_dist_are_byte_reproducible(self) -> None:
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary:
            ...
            self.assertEqual(
                set(checksums),
                {"codex/ask-then-do-it-1.4.3.zip", "generic/ask-then-do-it-generic-1.4.3.zip", "claude/ask-then-do-it-claude-1.4.3.zip"},
            )
            for relative, digest in checksums.items():
                self.assertEqual(sha256(first / relative), digest)

            for directory, archive, archive_root in (
                ("claude/ask-then-do-it", "claude/ask-then-do-it-claude-1.4.3.zip", "ask-then-do-it"),
                ("codex/ask-then-do-it", "codex/ask-then-do-it-1.4.3.zip", "ask-then-do-it"),
                ("generic/ask-then-do-it-generic-1.4.3", "generic/ask-then-do-it-generic-1.4.3.zip", "ask-then-do-it-generic-1.4.3"),
            ):
