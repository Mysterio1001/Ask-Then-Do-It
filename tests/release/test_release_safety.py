...
class ReleaseSafetyTests(unittest.TestCase):
    ...
    def test_complete_verified_prior_release_can_be_upgraded_atomically(self) -> None:
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary:
            ...
            self.assertTrue((output / "codex" / "ask-then-do-it-1.4.3.zip").is_file())
            self.assertTrue(
                (output / "generic" / "ask-then-do-it-generic-1.4.3.zip").is_file()
            )
            self.assertTrue((output / "claude" / "ask-then-do-it-claude-1.4.3.zip").is_file())
    ...
    def test_clean_builds_are_byte_reproducible_and_zips_match_directories(self) -> None:
        with tempfile.TemporaryDirectory(dir=ROOT) as temporary:
            ...
            for name in (
                ...
                "claude/ask-then-do-it-claude-1.4.3.zip",
                "checksums.sha256",
            ):
                self.assertEqual(
