...
class CleanSlateContractTests(unittest.TestCase):
    def test_all_active_component_versions_are_1_3_1(self) -> None:
        ...
        self.assertIn("Core version: `1.4.1`", text(ROOT / "core" / "CORE.md"))
        self.assertIn("core_version: 1.4.1", text(ROOT / "core" / "rules" / "rules.yaml"))
        ...
        for manifest in (
            ...
            with self.subTest(manifest=manifest.relative_to(ROOT)):
                ...
                self.assertIn("1.4.1", text(manifest))
        ...
        for prompt in EXPECTED_PROMPTS:
            with self.subTest(prompt=prompt):
                self.assertIn("Core version: `1.4.1`", text(GENERIC / prompt))
    ...
    def test_release_configuration_uses_provider_directories(self) -> None:
        ...
        self.assertEqual(
            config["codex"]["archive"], "codex/ask-then-do-it-1.4.1.zip"
        )
        self.assertEqual(
